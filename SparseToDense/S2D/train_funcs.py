...
#             print(sample_dict['points_target'])
            ...
            loss = loss_fn(t_target, tx_hat, tx, displacement)
            ...
            #### Naima: Change here if you descard normalization
                with torch.no_grad():
                    if shapedata.mean.shape[0] != tx.shape[1]:
                        tx_norm = tx[:, :-1, :]
                        displacement_norm=displacement[:, :-1, :]
                        tx_hat_norm = tx_hat[:, :-1, :]
                        tx_target_norm = t_target[:, :-1, :]
                    else:
                        tx_norm = tx
                        displacement_norm=displacement
                        tx_hat_norm = tx_hat
                        tx_target_norm = tx_target
                                                

                    tx_norm = (tx_norm - shapedata_mean) / shapedata_std
                    tx_norm = torch.cat((tx_norm, torch.zeros(tx.shape[0], 1, tx.shape[2]).to(device)), 1)
                    
                    displacement_norm = (displacement_norm - shapedata_mean) / shapedata_std
                    displacement_norm = torch.cat((displacement_norm, torch.zeros(tx.shape[0], 1, tx.shape[2]).to(device)), 1)
                    ...
                    tx_target_norm=(tx_target_norm - shapedata_mean) / shapedata_std
                    tx_target_norm = torch.cat((tx_target_norm, torch.zeros(tx.shape[0], 1, tx.shape[2]).to(device)), 1)

                    loss_norm = loss_fn(tx_hat_norm, tx_target_norm, tx_norm, displacement_norm )
                ...
                if shapedata.normalization:
                    ...
                    with torch.no_grad():
                        if shapedata.mean.shape[0] != tx.shape[1]:
                            ...
                            displacement_norm = displacement[:, :-1, :]
                            tx_hat_norm = tx_hat[:, :-1, :]
                            t_target_norm = t_target[:, :-1, :]
                        else:
                            tx_norm = tx
                            displacement_norm= displacement
                            tx_hat_norm = tx_hat
                            t_target_norm = t_target
                        ...
                        t_target_norm =(t_target_norm - shapedata_mean) / shapedata_std
                        t_target_norm = torch.cat((t_target_norm, torch.zeros(t_target.shape[0], 1, t_target.shape[2]).to(device)), 1)

                        tx_hat_norm = (tx_hat_norm - shapedata_mean) / shapedata_std
                        tx_hat_norm = torch.cat((tx_hat_norm, torch.zeros(tx.shape[0], 1, tx.shape[2]).to(device)), 1)

                        loss_norm = loss_fn(tx_norm, t_target_norm, tx_hat_norm, displacement_norm)
            ...
            with torch.no_grad():
                if epoch == 0:
                    ...
                    shapedata.save_meshes(os.path.join(samples_dir, 'input_epoch_{0}'.format(epoch)),
                                          msh, mesh_ind)
                aa=random.randint(1,tx_hat.shape[0])    
                mesh_ind = [0] ## Changed to see what is generated during training
